...
const Casting = () => {
  ...
  const handleCreateNewShoot = async () => {
    ...
    const confirmSend = window.confirm(
      "Are you sure you want to create this new film shoot? This action is irreversible."
    );
    if (confirmSend) {
  ...
  const applyToFilmShootFunction = async () => {
    if (
      ...
      alert(
        "Please fill all your information and select at least one film shoot to apply for."
      );
      return;
    }
    setIsLoading(true);
    ...
    if (resp.message === "Sent in your application successfully") {
      ...
      alert(
        "Error submitting your application! If it persists please contact our team (hey@collegemixer.us)"
      );
    }
  };
    ...
    return (
      <div className="items-center justify-center p-2">
        ...
        <h1 className="text-center font-semibold text-2xl">
          Submitted your profile. Our casting team will be in touch!
        </h1>
        <div className="items-center justify-center flex flex-col gap-4 mt-16 mb-4">
          <button
            ...
            className="bg-slate-700 mx-auto m-4 rounded-lg p-3 text-white font-semibold transition-all duration-200 hover:bg-pink-600 hover:scale-95">
            Get the app
          </button>
        </div>
      </div>
  ...
  return (
    <div className="items-center justify-center p-2">
      ...
      <div
        className="sticky top-0 z-50 bg-white p-1"
        ...
        <div
          ...
          <div
            className="rounded-full duration-200 transition-all"
            style={{
              width: `${progress}%`,
              height: "100%",
      ...
      {adminCode === "zac" && (
        <div className="p-4 bg-gradient-to-r from-red-400 to-orange-700   rounded-lg">
          <h1 className="text-white  font-semibold text-4xl">
            Create New Shoot (admin)
          </h1>
          <div>
            <input
              type="datetime-local"
              placeholder="Date"
              className="rounded-md p-2 mt-4 h-8"
              value={shootDate}
              onChange={(n) => setShootDate(n.target.value)}></input>
          </div>
          <input
            placeholder="Shoot Name"
            className="rounded-md p-2 mt-4 h-8"
            value={shootName}
            onChange={(n) => setShootName(n.target.value)}></input>
          <div>
            <input
              placeholder="Shoot Location"
              className="rounded-md p-2 mt-4 h-8"
              value={shootLocation}
              onChange={(n) => setShootLocation(n.target.value)}></input>
          </div>
          <div className="flex flex-row justify-between">
            <input
              placeholder="Emoji Summary (2 emojis)"
              className="rounded-md p-2 mt-4 h-8"
              value={shootEmojiSummary}
              onChange={(n) => setShootEmojiSummary(n.target.value)}></input>
            <button
              onClick={handleCreateNewShoot}
              className="p-2 text-sm shadow-md border border-black shadow-slate-600 px-10 rounded-md bg-slate-800 text-white">
              Send
            </button>
          </div>
        </div>
      ...
      <h1 className="text-3xl font-bold my-4 mb-10 text-center">
        College Mixer Video Casting
      </h1>

      <div
        className={`p-4 py-4 mt-4 border shadow-inner border-gray-300 rounded-xl duration-100 transition-all ${
          name && isTyping !== "name"
        ...
        <div className="flex flex-row justify-between">
          ...
          <div
            className={`rounded-full text-center text-gray-700 w-12 px-3.5 bg-gray-100 border border-gray-500 p-2`}>
            {name && isTyping !== "name"
              ? "✓"
              : isTyping === "name"
              ? "..."
        ...
        <input
          ...
          placeholder="e.g., Zac"></input>
      </div>

      <div
        className={`p-4 py-4 mt-4 border shadow-inner border-gray-300 rounded-xl duration-100 transition-all ${
          major && isTyping !== "major"
            ? "bg-green-500 text-white"
            : isTyping === "major"
            ? "bg-yellow-200"
            : ""
        ...
        <input
          ...
          className="bg-white mt-4 border text-gray-800 border-gray-200 p-1 rounded-lg"
          placeholder="e.g., Kinesiology"></input>
      </div>

      <div
        className={`p-4 py-4 mt-4 border shadow-inner border-gray-300 rounded-xl duration-100 transition-all ${
          instagram && isTyping !== "instagram"
            ? "bg-green-500 text-white"
            : isTyping === "instagram"
            ? "bg-yellow-200"
            : ""
        ...
        <input
          ...
          className="bg-white mt-4 border text-gray-800 border-gray-200 p-1 rounded-lg"
          placeholder="e.g., @college_mixer"></input>
      </div>

      <div
        className={`p-4 py-4 mt-4 border shadow-inner border-gray-300 rounded-xl duration-100 transition-all ${
          phoneNumber && isTyping !== "phoneNumber"
            ? "bg-green-500 text-white"
        ...
        <input
          ...
          className="bg-white mt-4 border text-gray-800 border-gray-200 p-1 rounded-lg"
          placeholder="e.g., 905-505-1134"></input>
      </div>

      <div
        className={`p-4 py-4 mt-4  border shadow-inner border-gray-300 rounded-xl duration-100 transition-all ${
          selectedFilmShoots.length > 0 && isTyping !== "filmShoot"
            ? "bg-green-500 text-white"
            : isTyping === "filmShoot"
            ? "bg-yellow-200"
            : ""
        ...
        <div className="grid grid-cols-2 gap-2 my-4">
          {availableFilmShoots.length > 0 ? (
            availableFilmShoots.map((shoot: any, index: any) => (
              <div
                onClick={() => {
                  setSelectedFilmShoots(
                    selectedFilmShoots.includes(shoot._id)
                ...
                key={index}
                className={`rounded-lg h-36 w-full p-2 transition-all duration-200 cursor-pointer border  shadow-md my-2 ${
                  selectedFilmShoots.includes(shoot._id)
                ...
                <h2 className="font-bold text-sm">
                  {new Intl.DateTimeFormat("en-US", {
                    ...
                    month: "long",
                    day: "numeric",
                  }).format(new Date(shoot.shootDate))}
                </h2>
          ...
          <div
            onClick={() => {
              setSelectedFilmShoots(
                selectedFilmShoots.includes("n/a-other")
            ...
            className={`rounded-lg h-32 w-full p-2 transition-all duration-200 cursor-pointer border  shadow-md my-2 ${
              selectedFilmShoots.includes("n/a-other")
                ? "bg-slate-900  text-white border-slate-700"
                : " border-gray-200"
            }`}>
            ...
            <h4
              ...
              If no other times work, select this option
            </h4>
      ...
      <div
        onClick={applyToFilmShootFunction}
        className="p-2 cursor-pointer rounded-lg w-24 border-blue-500 border text-center text-white font-semibold bg-blue-400 shadow-md hover:bg-blue-500 mx-auto mt-4">
              ...
              availableFilmShoots.map((shoot: any, index: any) => (
                <div
                  onClick={() => {
                    setSelectedFilmShoots(
                      selectedFilmShoots.includes(shoot._id)
                        ? selectedFilmShoots.filter(
                            (id: any) => id !== shoot._id
                          ) // Remove the shoot._id if it's already included
                        : [...selectedFilmShoots, shoot._id] // Add the shoot._id if it's not included
                    );
                  }}
                  key={index}
