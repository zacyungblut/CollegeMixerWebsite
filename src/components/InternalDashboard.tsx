...
const Dashboard = () => {
  ...
  const handleSendClick = async () => {
    ...
    const confirmSend = window.confirm(
      "Are you sure you want to send this notification? This action is irreversible."
    );
    if (confirmSend) {
    ...
    <div className="items-center justify-center p-2">
      ...
      <div className="p-4 bg-gradient-to-r from-sky-600 to-sky-700   rounded-lg">
        ...
        <input
          ...
          onChange={(n) => setNotficationSubject(n.target.value)}></input>
        <div className="flex flex-row justify-between">
          <input
            placeholder="Message"
            className="rounded-md p-2 mt-4 h-8"
            value={notificationMessage}
            onChange={(n) => setNotficationMessage(n.target.value)}></input>
          <button
      ...
      <div className="flex-row flex mt-8 gap-2">
        ...
        <div className="border gap-4 rounded-lg w-1/2 p-2 flex flex-col items-center">
          <div className="flex  items-center">
            ...
            <h3 className="text-5xl text-blue-800 font-semibold">
              {maleCount || ""}
            </h3>
          </div>
          <div className="flex items-center">
            <h3>Number of Girls:&nbsp;</h3>
            <h3 className="text-5xl text-red-700 font-semibold">
              {femaleCount || ""}
            </h3>
          </div>
        </div>
